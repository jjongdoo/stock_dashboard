...
# 1. 깃허브 CSV 파일 읽기 (종목명 매핑용)
@st.cache_data
def load_krx_mapping():
    try:
        return pd.read_csv('krx_mapping.csv', dtype={'Code': str})
    except Exception:
        st.error("🚨 krx_mapping.csv 파일을 찾을 수 없습니다. 영문 티커를 사용해주세요.")
...
if user_input:
    ...
    # 🌟 연간(3년) / 분기(5년) 동적 기간 설정
    if "연간" in period_option:
        band_years = 3
    else:
        band_years = 5

    # 🚀 네이버 금융 크롤링 (현재 PER/PBR 및 과거 EPS/BPS 추출)
    @st.cache_data(ttl=3600)
    def get_naver_fundamentals(pure_sym):
        url = f"https://finance.naver.com/item/main.naver?code={pure_sym}"
        ...
        try:
            ...
            def extract_val(selector):
                try: return float(soup.select_one(selector).text.replace(',', ''))
                except: return np.nan

            c_per = extract_val('#_per')
            c_pbr = extract_val('#_pbr')
            c_roe = (c_pbr / c_per) if (pd.notna(c_per) and c_per > 0 and pd.notna(c_pbr)) else np.nan

            # 2. 밴드 계산을 위한 과거 EPS, BPS 긁어오기
            tables = pd.read_html(res.text, encoding='euc-kr')
            eps_dict, bps_dict = {}, {}
            for tb in tables:
                if isinstance(tb.columns, pd.MultiIndex) and '최근 연간 실적' in tb.columns.get_level_values(0):
                    tb.columns = tb.columns.droplevel(0)
                    tb = tb.set_index(tb.columns[0])
                    
                    if 'EPS(원)' in tb.index:
                        for k, v in tb.loc['EPS(원)'].dropna().items():
                            yr = str(k).split('.')[0]
                            if yr.isdigit() and str(v).replace(',','').replace('.','').isdigit():
                                eps_dict[int(yr)] = float(str(v).replace(',', ''))
                    
                    if 'BPS(원)' in tb.index:
                        for k, v in tb.loc['BPS(원)'].dropna().items():
                            yr = str(k).split('.')[0]
                            if yr.isdigit() and str(v).replace(',','').replace('.','').isdigit():
                                bps_dict[int(yr)] = float(str(v).replace(',', ''))
                    break
            return c_per, c_pbr, c_roe, eps_dict, bps_dict
        except Exception:
    ...
    # 데이터 호출
    hist_band = ticker.history(period=f"{band_years}y") # 밴드용 주가 (3년 or 5년)
    hist_1y = hist_band.tail(252) if not hist_band.empty else pd.DataFrame() # 성과분석용 1년 주가
    ...
    # 🚀 한국 주식: 크롤링 지표 적용 & 밴드 역산 / 미국 주식: 야후 지표 적용
    hist_per, hist_pbr = pd.Series(dtype=float), pd.Series(dtype=float)
    
    if is_korean:
        disp_per, disp_pbr, disp_roe, eps_dict, bps_dict = get_naver_fundamentals(pure_ticker)
        
        # 자체 알고리즘: 매일의 주가를 해당 연도의 EPS/BPS로 나누어 밴드 점 생성
        if not hist_band.empty and eps_dict and bps_dict:
            hist_per = pd.Series(index=hist_band.index, dtype=float)
            hist_pbr = pd.Series(index=hist_band.index, dtype=float)
            for date, row in hist_band.iterrows():
                yr = date.year
                # 당해년도 데이터가 없으면 전년도 데이터 사용 (자연스러운 밴드 형성)
                eps = eps_dict.get(yr) or eps_dict.get(yr-1) or eps_dict.get(yr-2)
                bps = bps_dict.get(yr) or bps_dict.get(yr-1) or bps_dict.get(yr-2)
                
                if eps and eps > 0: hist_per[date] = row['Close'] / eps
                if bps and bps > 0: hist_pbr[date] = row['Close'] / bps
    else:
        disp_per = stats.get('trailingPE', 'N/A')
        disp_pbr = stats.get('priceToBook', 'N/A')
        disp_roe = stats.get('returnOnEquity', "N/A")

    # 리스크 지표 (Beta, Sharpe, CAPM)
    beta, sharpe_ratio, capm_return = np.nan, np.nan, None
    ...
    # 🌟 탭(Tabs) 분리
    tab1, tab2, tab3 = st.tabs(["📊 가격 & 재무 차트", f"📈 밸류에이션 ({band_years}년 밴드) & 목표가", "⚖️ 리스크 & 듀퐁 분석"])
    ...
    with tab1:
        ...
        with st.expander(f"상세 재무 데이터 표 보기 (단위: {unit_text})"):
    ...
    with tab2:
        ...
        # 🌟 자체 계산된 3년/5년 동적 밴드 차트
        if is_korean and not hist_per.empty and not hist_pbr.empty:
            st.subheader(f"📊 역사적 PER / PBR 밴드 (최근 {band_years}년 일간 추이)")
            st.markdown("네이버 금융의 과거 EPS/BPS와 매일의 주가를 파이썬으로 직접 역산하여 구현한 프리미엄 밴드입니다.")
            
            fig_val = make_subplots(specs=[[{"secondary_y": True}]])
            fig_val.add_trace(go.Scatter(x=hist_per.index, y=hist_per, name='PER (배)', mode='lines', line=dict(color='blue', width=1.5)), secondary_y=False)
            fig_val.add_trace(go.Scatter(x=hist_pbr.index, y=hist_pbr, name='PBR (배)', mode='lines', line=dict(color='purple', width=1.5)), secondary_y=True)
            ...
            st.warning("⚠️ 해당 종목은 적자 누적 등의 이유로 과거 EPS/BPS 데이터를 산출할 수 없어 밴드 생성이 불가능합니다.")
